...
export async function getDashboardData() {
  ...
  // ⚡ Bolt: Consolidated getServerSession to prevent redundant CPU-intensive JWT decryptions
  ...
  const [totalBalance , totalIncome , totalExpenses , totalInvestments, totalLoans] = await Promise.all([
    getTotalBalance(start, end, token),
    getTotalIncome(start, end, token),
    getTotalExpenses(start, end, token),
    getTotalInvestments(start, end, token),
    getTotalLoans(start, end, token),
  ])
