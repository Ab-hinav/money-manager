...
export async function getDashboardData() {
  // ⚡ Bolt: Consolidate getServerSession calls to prevent redundant CPU-intensive JWT decryption
  // We retrieve the session once at the top level and pass the accessToken to helper functions.
  ...
  const [totalBalance , totalIncome , totalExpenses , totalInvestments, totalLoans] = await Promise.all([
    getTotalBalance(session.accessToken, startOfMonth, endOfMonth),
    getTotalIncome(session.accessToken, startOfMonth, endOfMonth),
    getTotalExpenses(session.accessToken, startOfMonth, endOfMonth),
    getTotalInvestments(session.accessToken, startOfMonth, endOfMonth),
    getTotalLoans(session.accessToken, startOfMonth, endOfMonth),
  ])
