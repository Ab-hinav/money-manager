...
func (h *DashboardHandler) GetTotalSavings(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		// ⚡ Bolt: Calculate total within primary loop to avoid O(N) secondary iteration and type assertions
		totalSavings += amount
	}
	if err := rows.Err(); err != nil {
		log.Println("Rows iteration error", err)
...
func (h *DashboardHandler) GetTotalLoans(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		loans[name] += amount
		// ⚡ Bolt: Calculate total within primary loop to avoid O(N) secondary iteration
		totalLoans += amount
	}
	if err := rows.Err(); err != nil {
		log.Println("Rows iteration error", err)
